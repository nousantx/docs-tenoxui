...
There's still no stable or correct ways to use tenoxui, we still experimenting for the best usage of tenoxui. Since it uses `DOM` to work with, the styles must loaded immediately after the page fire up. But, try tenoxui with `react + vite`. Here's the simple boilerplate you can ise
